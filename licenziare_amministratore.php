...
<html>
    <meta charset="UTF-8">
    <title>Licenzia amministratore</title>
    ...
    <body>
        ...
        <div class="w3-bar-block w3-animate-left sidebar" style="display:none;z-index:5;" id="mySidebar">
            ...
            <div id="demoAcc" class="w3-hide w3-card  href-accordion">
                ...
                <a href="licenziare_amministratore.php" class="w3-bar-item w3-button elementoAccordion bottoneCliccato">Licenziamento amministrazione</a>
            </div>
        ...
        <div>
            ...
            <div class="w3-container">
                <h1 style="text-align: center;">Licenzia amministratore</h1>
                <form action="" method="post" style="text-align: center">
                    Selezionare amministratore da licenziare:
                    <table align="center">
                        <?php
                        // chiamata alla funzione di connessione
                        $conn = connetti();
                        ...
                        $qry = "SELECT * FROM amministrazione WHERE licenziato='no' AND id_amministrazione!='".$_SESSION["id"]."' ORDER BY nome;";
                        $result = $conn->query($qry);
                        ...
                        if ($result->num_rows > 0) {
                            ...
                            while ($row = $result->fetch_assoc()) {
                                echo "<tr><td>" . $row['nome'] . "</td><td>" . $row['cognome'] . "</td><td>" . $row['data_nascita'] . "</td><td><input type='checkbox' name='seL[]' value='" . $row['id_amministrazione'] . "'></input></td></tr>";
                            }
                            echo "<tr><td colspan='4'><input style='width: 100%;' type='submit' value='Licenzia' class='bottone'></td></tr>";
                        } else {
                            //se non ci sono risultati vuol dire che non ci sono meccanici
                            echo "<h1>SEI L'UNICO AMMINISTRATORE</h1>";
                        }
                        ?>
                    </table>
                </form>
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($_POST['seL'])) {
                foreach ($_POST['seL'] as $key => $l) {
                    ...
                    $qry = "SELECT count(*) as allcount FROM amministrazione WHERE licenziato='si' AND id_amministrazione=$l";
                    $result = $conn->query($qry);
                    $row = mysqli_fetch_array($result);
                    $allcount = $row['allcount'];

                    if ($allcount == 0) {
                        $qry = "UPDATE amministrazione SET licenziato='si' WHERE id_amministrazione='$l' AND id_amministrazione!='".$_SESSION["id"]."'";
                        if (!$conn->query($qry)) {
                            die("<script>alert('Errore della query: " . $conn->error . ", " . $conn->error . ".')</script>");
                        } else {
                            $qry = "DELETE FROM registrazione_amministrazione WHERE fk_id_amministrazione='$l'";
                            if (!$conn->query($qry)) {
                                die("<script>alert('Errore della query: " . $conn->error . ", " . $conn->error . ".')</script>");
                            } else {
                                ...
                                echo "<script> window.location.replace('licenziare_amministratore.php') </script>";
                            }
                        }
                    } else {
