...
<html>
    <meta charset="UTF-8">
    <title>Registrazione modello</title>
    ...
    <body>
        ...
        <div class="w3-bar-block w3-animate-left sidebar" style="display:none;z-index:5;" id="mySidebar">
            ...
            <div id="demoAcc" class="w3-hide w3-card href-accordion">
                ...
                <a href="registrazione_modello.php" class="w3-bar-item w3-button  elementoAccordion bottoneCliccato">Registrazione nuovo modello</a>
            </div>
        ...
        <div>
            ...
            <div class="w3-container">
                <h1 style="text-align: center;">Modello</h1>

                <form method="post" action="javascript:loadDoc();">
                    <table>
                        <tr>
                            <td>Marca<span style="color: #990000;">*</span>:</td><td>
                                <select id="marca" required>
                                    <option id="0" value="0">Scegliere una marca</option>
                                    <?php
                                    // chiamata alla funzione di connessione
                                    $conn = connetti();

                                    // costruzione ed esecuzione query
                                    $qry = "SELECT * FROM marca ORDER BY nome;";
                                    $result = $conn->query($qry);
                        ...
                        <tr>
                            <td>Nome<span style="color: #990000;">*</span>:</td><td><input type="text" id="nome" placeholder="Inserire un nome"  maxlength="20" required></td>
                        </tr>
                        <tr>
                            <td>Tipo<span style="color: #990000;">*</span>:</td><td><select id="tipo" required>
                        ...
                        <tr>
                            <td>Peso(kg)<span style="color: #990000;">*</span>:</td><td><input type="number" id="peso" value="0" max="10000.0" min="10"  required></td>
                        </tr>
                        <tr>
                            <td>Cambio automatico<span style="color: #990000;">*</span>:</td><td><select id="cambio" required>
                        ...
                        <tr>
                            <td>Alimentazione<span style="color: #990000;">*</span>:</td><td><select id="alimen" required>
                                    <option value="-1">Scegli alimentazione</option>
                        ...
                        <tr>
                            <td>Cilindrata(litri)<span style="color: #990000;">*</span>:</td><td> <input type="number" id="cilin" min="0.1" max="10.0" step="0.1" value="0" required></td>
                        </tr>
                        <tr>
                            <td colspan="2"><input type="submit" value="Invia" class="bottone"></td>
                        </tr>	
                    </table>
                </form> 
            </div>
            ...
            <script>
                function loadDoc() {
                    if ((document.getElementById("marca").value != "0") && (document.getElementById("alimen").value != "-1") && (document.getElementById("cambio").value != "-1")) {
                        ...
                        xhttp.onreadystatechange = function () {
                            if (this.readyState == 4 && this.status == 200) {
                                ...
                                if (risp == "Registrazione effettuata correttamente.") {
                                    ...
                                    document.getElementById("alimen").selectedIndex = "0";
                                    document.getElementById("cilin").value = "0";
                                }
                            }
                        };
                        ...
                        var alimen = document.getElementById("alimen").value;
                        var cilin = document.getElementById("cilin").value;

                        query = "INSERT INTO modello (nome, tipo, peso, cambio_automatico, alimentazione, cilindrata, fk_id_marca) VALUES ('" + nome + "', '" + tipo + "', '" + peso + "', '" + cambio + "', '" + alimen + "', '" + cilin + "', '" + marca + "' )";
                        //cripto la password
                        //devo ancora capire come fare
                        xhttp.open("GET", "inserimentoDB.php?query=" + query, true);
                        xhttp.send();
                    } else {
                        alert("non hai selezionato tutti i campi");
                    }
                }
                //apro la sidebar
                function w3_open() {
